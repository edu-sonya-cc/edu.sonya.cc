...
function (_super) {
  ...
  function BrickCore() {
    ...
    _this.getForePageHtml = function () {
      ...
      var CHARS = [];
      (useSameBackCover ? chars : charsNotSameBackCover).forEach(function (_char) {
        return CHARS.push(_char);
      });
      ...
      var BOTTOM_START = "<bottom>",
          BOTTOM_END = "</bottom>";
      var TEXT_END = "</text>";
      var TEXT_START_TOP_LEFT = '<text class="top-left">'; // const TEXT_START_BOTTOM_LEFT  = '<text class="bottom-left">';
        ...
        for (var loopOfRow = 0; loopOfRow < ROW_COUNT; ++loopOfRow) {
          ...
          for (var loopOfCol = 0; loopOfCol < COLUMN_COUNT; ++loopOfCol) {
            ...
            if (symbolIndex <= MAX_SYMBOL_INDEX) {
              var _char2 = CHARS[symbolIndex] || "";

              var useKaiti = "ˉ,ˊ,ˇ,ˋ".indexOf(_char2) > -1; // html += (useKaiti ? TEXT_START_TOP_LEFT_USE_KAITI : TEXT_START_TOP_LEFT).concat(char, TEXT_END);
              // html += (useKaiti ? TEXT_START_BOTTOM_RIGHT_USE_KAITI : TEXT_START_BOTTOM_RIGHT).concat(char, TEXT_END);

              html += TOP_START.concat(useKaiti ? TEXT_START_TOP_LEFT_USE_KAITI : TEXT_START_TOP_LEFT, _char2, TEXT_END, TOP_END);
              html += BOTTOM_START.concat(useKaiti ? TEXT_START_BOTTOM_RIGHT_USE_KAITI : TEXT_START_BOTTOM_RIGHT, _char2, TEXT_END, BOTTOM_END);
            }
    ...
    _this.INITIAL_ARRAY = "b,p,m,f,d,t,n,l,g,k,h,j,q,x,zh,ch,sh,r,z,c,s,y,w,".split(","); // 24个韵母

    _this.VOWEL_ARRAY = "a,o,e,i,u,ü,ai,ei,ui,ao,ou,iu,ie,üe,er,an,en,in,un,ün,ang,eng,ing,ong".split(","); // 16个整体认读

    _this.OVERALL_READING_ARRAY = "zhi,chi,shi,ri,zi,ci,si,yi,wu,yu,ye,yue,yuan,yin,yun,ying".split(","); // // 4个声调（2份）加一个空白
    ...
    _this.THREE_SYLLABLE_SPELLING_AND_TONE_ARRAY = "ia,ua,uo,uai,iao,ian,iang,uan,uang,iong,üan,ˉ,ˊ,ˇ,ˋ,ˉ,ˊ,ˇ,ˋ,".split(","); // 30个带声调单韵母，Word需替换[āáǎàōóǒòēéěèīíǐìūúǔùǖǘǚǜ]
    ...
    _this.countPokerDataAndComputedData = function (pokerKind, countPerPage) {
      ...
      _this.computedData.count = Math.ceil(CHARS.length / countPerPage) * countPerPage;
    ...
    _this.initPokerKindElements = function (wrapElement) {
      var _a = _this,
          ...
          pokerKindElementArray = _a.pokerKindElementArray; // const labelElement = createElement('label') as HTMLLabelElement;
      ...
      var pokerKindI18nHtmlArray = [getI18nInnerHTML({
        en_us: "Initials",
        zh_cn: "声母",
        zh_tw: "聲母"
      }), getI18nInnerHTML({
      ...
      var _loop_1 = function _loop_1(pokerKindIndex) {
        var pokerKindValue = Math.pow(2, pokerKindIndex);
    ...
    _this.initUseSameBackCoverElements = function (wrapElement) {
      var _a = _this,
          ...
          useSameBackCoverElementArray = _a.useSameBackCoverElementArray; // const labelElement = createElement('label') as HTMLLabelElement;
      // wrapElement.appendChild(labelElement);
      // labelElement.innerHTML = getI18nInnerHTML({
      //   en_us: '',
      //   zh_cn: '',
      ...
      var i18nHtmlArray = [getI18nInnerHTML({
        en_us: "Yes",
        zh_cn: "是",
        zh_tw: "是"
      }), getI18nInnerHTML({
  ...
  BrickCore.prototype.countIt = function (enAppend, zh_cnAppend, zh_twAppend, charsArray, enFullArray, zh_cnArray, zh_twArray, en_us, zh_cn, zh_tw, CHARS, CHARS_NOT_SAME_BACK_COVER, countPerPage, BACK_COVERS) {
    ...
    charsArray.forEach(function (_char3) {
      CHARS.push(_char3);
      CHARS_NOT_SAME_BACK_COVER.push(_char3);
    });
    var arrayLength = charsArray.length;
    var countNotSameBackCoverIncrease = countPerPage * Math.ceil(arrayLength / countPerPage);
