  ...
  return {
    ...
    execute: function execute() {
        ...
        function PcGlobal() {
          ...
          this.onAddFavorite = function (event) {
            ...
            switch (ACTUAL_PAGE_NAME) {
              ...
              case "bricks":
                url = HOME_URL + "?go=" + ACTUAL_PAGE_NAME + "&kind=" + _this.getPageSubKind() + "&page=" + _this.getPageIndex();
                break;
          ...
          this.onShare = function (event) {
            console.log("onShare()");
            var url = encodeURI(window.location.href);
            var wechatShareImageSrcPostfix = url.indexOf("&") === -1 ? "" : "/".concat(url.split("&").slice(1).map(function (keyValue) {
          ...
          this.changePaginationParams = function (list, pageSize, listElement, paginationElement, fillItem) {
            ...
            var gotoPage = function gotoPage(pageIndex) {
              if (pageIndex > pageMaxIndex) pageIndex = pageMaxIndex;else if (pageIndex < 0) pageIndex = 0;
              ...
              var pageNumberElementMaxIndex = pageNumbersWrapElement.children.length - 1;
              ...
              var fullUrl = url.indexOf("&page=") === -1 ? url.concat(pageSeg) : url.replace(/&page=[0-9]+/g, pageSeg);
      ...
      document.onclick = function () {
        querySelectorAll(".topMenuItemSubMenuWrap,#shareArea,#brickPageShareArea,#brickPageSponsorImage").forEach(function (element) {
          return hide(element);
        });
      };
    }
...
__exp = _instantiate("pcGlobal", false);
