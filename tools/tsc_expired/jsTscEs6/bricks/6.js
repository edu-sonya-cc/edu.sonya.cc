...
class BrickCore extends BrickWithTableBase {
  ...
  constructor() {
    super({}, {});
    this.idOrClassPrefix = "brickPageCuisenaireRod";
    /** OK
    ...
    this.countDataAndComputedData = () => {
      ...
      list.forEach(
        (
          ...
          const LAST_INNER_LINE_STYLE =
            innerLineStyleArray[INNER_LINE_STYLE_COUNT - 1];
          const LAST_CUT_LINE_STYLE =
            cutLineStyleArray[CUT_LINE_STYLE_COUNT - 1];
          const LAST_OUTER_LINE_STYLE =
          ...
          const VERTICAL_RODS_COL_COUNT = Math.floor(
            MAX_X / CUISENAIRE_ROD_MIN_WIDTH,
          );
          const CUISENAIRE_ROD_WIDTH = VERTICAL_RODS_COL_COUNT === 0
            ? 0
            : Math.min(
          ...
          pages.forEach((PAGE_COUNT, numberIndex) => {
            const innerLineStyle = numberIndex < INNER_LINE_STYLE_COUNT
              ? innerLineStyleArray[numberIndex]
              : LAST_INNER_LINE_STYLE;
            const cutLineStyle = numberIndex < CUT_LINE_STYLE_COUNT
              ? cutLineStyleArray[numberIndex]
              : LAST_CUT_LINE_STYLE;
            const outerLineStyle = numberIndex < OUTER_LINE_STYLE_COUNT
              ? outerLineStyleArray[numberIndex]
              : LAST_OUTER_LINE_STYLE;
            const textStyle = numberIndex < TEXT_STYLE_COUNT
              ? textStyleArray[numberIndex]
              : LAST_TEXT_STYLE;
            const NUMBER_MAX = numberIndex + 1;
            // 'none' | 'onlyAbove' | 'onlyBelow' | 'both'
            const CELL_COUNT_BY_SEALING_STYLE = sealingStyle === "both"
              ? 2
              : (sealingStyle === "none" ? 0 : 1);
            const CUISENAIRE_ROD_HEIGHT = length * NUMBER_MAX +
              CELL_HEGIHT *
                (NUMBER_MAX === 1
                  ? (sealingStyle === "none" ? 0 : 1)
                  : CELL_COUNT_BY_SEALING_STYLE);
            const VERTICAL_RODS_ROW_COUNT = Math.floor(
              MAX_Y / CUISENAIRE_ROD_HEIGHT,
            );
            const VERTICAL_RODS_COUNT = VERTICAL_RODS_COL_COUNT *
              VERTICAL_RODS_ROW_COUNT;
            const HORIZONTAL_RODS_START_Y = CUISENAIRE_ROD_HEIGHT *
              VERTICAL_RODS_ROW_COUNT;
            const REMAINING_HEIGHT = MAX_Y - HORIZONTAL_RODS_START_Y;
            const HORIZONTAL_RODS_ROW_COUNT = Math.floor(
              REMAINING_HEIGHT / CUISENAIRE_ROD_MIN_WIDTH,
            );
            const HORIZONTAL_RODS_COL_HEIGHT = HORIZONTAL_RODS_ROW_COUNT === 0
              ? 0
              : Math.min(
                MAX_X / VERTICAL_RODS_COL_COUNT,
                CUISENAIRE_ROD_BEST_FIT_WIDTH,
            ...
            const HORIZONTAL_RODS_COL_COUNT = Math.floor(
              MAX_X / CUISENAIRE_ROD_HEIGHT,
            );
            const HORIZONTAL_RODS_COUNT = HORIZONTAL_RODS_COL_COUNT *
              HORIZONTAL_RODS_ROW_COUNT;
            const RODS_COUNT_PER_PAGE = VERTICAL_RODS_COUNT +
              HORIZONTAL_RODS_COUNT;
