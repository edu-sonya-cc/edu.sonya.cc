...
class BrickCore extends BrickWithTableBase {
    ...
    countDataAndComputedData = ()=>{
        ...
        const { paperSize , isLandscape , maxX: MAX_X , maxY: MAX_Y , pageMarginTop , pageMarginBottom , pageMarginLeft , pageMarginRight , list  } = data;
		...
		* { box-sizing:border-box; }
		...
		`;
        let html = '';
        list.forEach(({ onlyWhitePaper , digitalOverlay , sealingStyle , length , innerLineStyle: innerLineStyleArray , cutLineStyle: cutLineStyleArray , outerLineStyle: outerLineStyleArray , textStyle: textStyleArray , pages , paperThickness  })=>{
            pages.length;
    ...
    getHorizontalRodHtml = (digitalOverlay, startNumberOffset, NUMBER_MAX, sealingStyle, length, CUISENAIRE_ROD_WIDTH, CUISENAIRE_ROD_HEIGHT, mmToPxScale, INNER_LINE_STYLE, CUT_LINE_STYLE, OUTER_LINE_COLOR, TEXT_STYLE, paperThickness)=>{
        const { createSvg , appendLine , appendText , getTextStyleFontSizePatchCss , appendOuterLine  } = svgSpace.edu.sonya.cc.SvgHelper;
    ...
    createTableBodyRow = (item)=>{
        const { onlyWhitePaper , digitalOverlay , sealingStyle , length , innerLineStyle , cutLineStyle , outerLineStyle , textStyle , pages , paperThickness  } = item;
