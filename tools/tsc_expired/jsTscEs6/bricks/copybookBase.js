...
export class CopybookBase extends BrickBase {
  constructor(appendData, otherComputedData) {
    super(
      ...
      Object.assign({ css: "", html: "" }, otherComputedData),
    );
    /**
     ...
     * <zh_tw>字帖相關新數據</zh_tw>
     */
    this.updateOtherDataOfCopybook = (_newData) => {};
    /**
     * <en>
     ...
     * </zh_tw>
     */
    this.usableCopybookCheckboxElementArray = [];
    /**
     * <en>
     ...
     * </zh_tw>
     */
    this.usableCopybooksPeopleEducationEdition = [];
    /**
    ...
    this.splitPinyin = (pinyinBill, pinyinArray, charCount) => {
    ...
    this.fixPinyinArray = (pinyinBill, pinyinArray, charCount) => {
    ...
    this.countDataAndComputedData = () => {
      ...
      const TEXT_BOTTOM = (RECTANGLE_WIDTH - TEXT_WIDTH) * 0.5 -
        22 / 54 * SCALE;
    ...
    chinese{width:${TEXT_WIDTH}mm;height:${TEXT_HEIGHT}mm;left:${TEXT_LEFT}mm;bottom:${TEXT_BOTTOM}mm;}
      ...
      const HTML_SUBTITLE = `<span class="subtitle">${
        i18nSubtitle[LANG]
      }</span>`;
      ...
      const answerPageCount = (answerHtml.split("</page>").length - 1)
        .toString();
      const html = questionHtml.replace(
        /~reporterPageCount~/g,
        questionPageCount,
    ...
    this.idOrClassPrefix = "brickPageCopybook";
    this.textareaChinese = createElement("textarea");
    this.textareaPinyin = createElement("textarea");
    this.textareaChineseAndPinyin = createElement("textarea");
    this.initCoreElements = () => {
      ...
      usableCopybooksPeopleEducationEdition.forEach(({ termI18n, units }) => {
        ...
        spanGroup.className =
          `${idOrClassPrefix}UsableCopybookCheckboxGroupWrap`;
        units.forEach((unit) => {
    ...
    this.updateChineseOrPinyinTextarea = (
      ...
      for (let i = 0; i < maxCount; ++i) {
        ...
        const pinyin = (i < pinyinLength ? pinyinArray[i] : "").replace(
          /a/g,
          "ɑ",
        ).replace(/g/g, "ɡ");
        copybooks.push({ chinese, pinyin });
