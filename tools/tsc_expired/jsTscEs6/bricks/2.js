...
class BrickCore extends BrickWithTableBase {
  ...
  constructor() {
    ...
    this.idOrClassPrefix = "brickPageMathStage";
    this.updateOtherDataLevel3 = (newData) => {
      const {
        onlyMentalArithmetic,
        pageSubjectFontSize,
        questionFontSize, // innerLineStyle,
        // outerLineStyle,
        // year,
        // month,
        // day,
      ...
      ["1-5", "0-5", "0-10", "0-20"].forEach((scope) => {
        ...
        for (let a = min; a <= max; ++a) {
          ...
          for (let b = min; b <= bMax; ++b) {
            ...
            const answer =
              `<p>${commonHtml}<answer chars="2">${result}</answer></p>`;
            list.push({
              question,
...
p[edu-right-char="9"]{padding-right:9em;}
      ...
      const i18nSubtitle = {
        en:
          `<span class="subtitleDay">${day}</span> <span class="subtitleMonth">${month}</span>, <span class="subtitleYear">${year}</span>`,
        zh_cn:
          `<span class="subtitleYear">${year}</span>年<span class="subtitleMonth">${month}</span>月<span class="subtitleDay">${day}</span>日`,
        zh_tw:
          `<span class="subtitleYear">${year}</span>年<span class="subtitleMonth">${month}</span>月<span class="subtitleDay">${day}</span>日`,
      };
      // const i18nCopies = {
      ...
      const titlePostfix = `_${NOW.getFullYear()}${
        "0".concat((NOW.getMonth() + 1).toString()).substr(-2)
      }${"0".concat(NOW.getDate().toString()).substr(-2)}`.concat(
        `_${"0".concat(NOW.getHours().toString()).substr(-2)}${
          "0".concat(NOW.getMinutes().toString()).substr(-2)
        }${"0".concat(NOW.getSeconds().toString()).substr(-2)}`,
        list.length < 4
          ? "_".concat(list.map(({ kind }) => kind).join("_and_"))
          : "",
      );
      // const title = ''.concat(
      ...
      const questionPageStartHtml =
        `<page class="questionPage">${questionPageSubjectRowHtml}`;
      const answerPageStartHtml =
        `<page class="answerPage">${answerPageSubjectRowHtml}`;
      computedData.html = this.getReportHtml(
     ...
     * @returns
     */
    this.getReportHtml = (questionPageStartHtml, answerPageStartHtml) => {
      ...
      // 先计算独立分页项
      list.filter(({ independentPagination }) => independentPagination).forEach(
        (info) => {
      ...
      list.filter(({ independentPagination }) => !independentPagination)
        .forEach((info) => {
          this.appendReportElements(info, questionRowsArray, answerRowsArray);
        });
      ...
      const questionPageCount = (questionHtml.split("</page>").length - 1)
        .toString();
      const answerPageCount = (answerHtml.split("</page>").length - 1)
        .toString();
     ...
     * @returns
     */
    this.getIndependentPaginationHtml = (
      rowsOccupied,
      rows,
      pageStartHtml,
      pageIndex,
      ...
      if (this.data.onlyMentalArithmetic) {
        rowsOccupied = this.defaultRowsOccupied;
      }
      const { smallRowCountPerPage } = this;
      let html = pageStartHtml.replace(
        "~reporterPageIndex~",
        (++pageIndex).toString(),
      );
      ...
      rows.forEach((row) => {
        ...
        remainingRowCount = this.formatCentile(
          remainingRowCount - rowsOccupied,
        );
      });
      html += "</page>";
    ...
    /** */
    this.smallRowCountPerPage = 48;
    this.defaultRowCountPerPage = 25;
    this.defaultRowsOccupied = this.smallRowCountPerPage /
      this.defaultRowCountPerPage;
    /** OK
     *
     * @param rowArray
     * @param pageStartHtml
     * @returns
     */
    this.getDependentPagingHtml = (rowArray, pageStartHtml, pageIndex) => {
      if (!rowArray.length) {
        return "";
      }
    ...
    // 	return resultArray[getIntegerRandom(0, resultArray.length - 1)];
    // }
    this.plusOrSubtractDictionaryNotGreatThan100Array = [];
    this.getAddendWithCarry = (other) => {
    ...
    this.getAddendWithoutCarry = (other) => {
      const plusOrSubtractDictionaryNotGreatThan100 =
        this.plusOrSubtractDictionaryNotGreatThan100Array[other];
      if (typeof plusOrSubtractDictionaryNotGreatThan100 === "undefined") {
        return 0;
    ...
    this.getSubtractorWithDebitMinus = (minuend) => {
      const plusOrSubtractDictionaryNotGreatThan100 =
    ...
    this.getSubtractorWithoutDebitMinus = (minuend) => {
      const plusOrSubtractDictionaryNotGreatThan100 =
        this.plusOrSubtractDictionaryNotGreatThan100Array[minuend];
      if (typeof plusOrSubtractDictionaryNotGreatThan100 === "undefined") {
        return 0;
      ...
      const array = plusOrSubtractDictionaryNotGreatThan100
        .subtractorWithoutDebitMinusArray;
      const maxIndex = plusOrSubtractDictionaryNotGreatThan100
        .subtractorWithoutDebitMinusMaxIndex;
      if (maxIndex === 0) {
        return array[0];
      }
    ...
    this.questionElementHtmlAppendStart8 = '<div edu-flex="8">';
    this.questionElementHtmlAppendStart6 = '<div edu-flex="6">';
    this.questionElementHtmlAppendStart4 = '<div edu-flex="4">';
    this.questionElementHtmlAppendStart3 = '<div edu-flex="3">';
    this.getSimleHtmlOfAMultiplyB = (A, B, result) => {
      return `<p><number>${A}</number></p><p><operator>×</operator><number>${B}</number></p><hr><p><number>${result}</number></p>`;
    };
    this.isOnlyFirstIsNotZero = (numberal) => {
    ...
    this.fillElementArrayOfAPlusBThenC = (
      ...
      const questionElementHtml =
        `<p>${commonHtml}</p>${questionElementHtmlAppend}`; // .replace(/~charCount~/g, charsStr)
      ...
      if (!onlyMentalArithmetic) {
        ...
        const tensCarryHtml1 = tensCarry1
          ? '<carry edu-digit="tens">1</carry>'
          : "";
        const hundredsCarryHtml1 = hundredsCarry1
          ? '<carry edu-digit="hundreds">1</carry>'
          : "";
        const tensCarryHtml2 = tensCarry2
          ? '<carry edu-digit="tens">1</carry>'
          : "";
        const hundredsCarryHtml2 = hundredsCarry2
          ? '<carry edu-digit="hundreds">1</carry>'
          : "";
        const tensCarryHtml = tensCarry
          ? `<carry edu-digit="tens">${
            Math.floor((aDidits + bDidits + cDidits) / 10)
          }</carry>`
          : "";
        const hundredsCarryHtml = hundredsCarry
          ? `<carry edu-digit="hundreds">${
            Math.floor((aLastTowDigits + bLastTowDigits + cLastTowDigits) / 100)
          }</carry>`
          : "";
        answerElementHtml += this.questionElementHtmlAppendStart6;
        answerElementHtml += `<answer-option edu-chars="${charsStr}">`;
        answerElementHtml +=
          `<p></p><p><number>${A}</number></p><p><operator>+</operator><number>${B}</number>${hundredsCarryHtml1}${tensCarryHtml1}</p><hr><p><number>${A_B}</number></p>`;
        answerElementHtml += '<hr class="step"/><p></p>';
        answerElementHtml +=
          `<p></p><p><number>${A_B}</number></p><p><operator>+</operator><number>${C}</number>${hundredsCarryHtml2}${tensCarryHtml2}</p><hr><p><number>${result}</number></p>`;
        answerElementHtml += "</answer-option>";
        answerElementHtml +=
          `<answer-option edu-chars="${charsStr}"><p></p><p></p>`;
        answerElementHtml +=
          `<p><number>${A}</number></p><p><operator>+</operator><number>${B}</number>${hundredsCarryHtml1}${tensCarryHtml1}</p><hr><p><number>${A_B}</number></p>`;
        answerElementHtml +=
          `<p><operator>+</operator><number>${C}</number>${hundredsCarryHtml2}${tensCarryHtml2}</p><hr><p><number>${result}</number></p>`;
        answerElementHtml += "</answer-option>";
        answerElementHtml +=
          `<answer-option edu-chars="${charsStr}"><p></p><p></p><p></p><p>`;
        answerElementHtml +=
          `<number>${A}</number></p><p><number>${B}</number></p>`;
        answerElementHtml +=
          `<p><operator>+</operator><number>${C}</number>${hundredsCarryHtml}${tensCarryHtml}</p><hr><p><number>${result}</number></p>`;
        answerElementHtml += "</answer-option>";
    ...
    this.fillElementArrayOfASubtractBThenC = (
      ...
      if (withBrackets) {
        ...
        if (has) {
          ...
          if (bPlusC % 10 === 0) {
            a = bPlusC === 100
              ? 100
              : 10 * getIntegerRandom(Math.ceil(bPlusC / 10), 10);
          } else {
      ...
      const commonHtml = withBrackets
        ...
        : `${A}<i> </i>-<i> </i>${B}<i> </i>-<i> </i>${C}<i> </i>=<i> </i>`;
      const questionElementHtml =
        `<p>${commonHtml}</p>${questionElementHtmlAppend}`; // .replace(/8/g, charsStr)
      let answerElementHtml = `<p>${commonHtml}${result}</p>`;
      if (onlyMentalArithmetic) {
        this.fillElementListCore(
          questionElementHtml,
      ...
      function doneBPlusCAndThenASubstractIt() {
        ...
        answerElementHtml +=
          `${debitHtml}<p><number>${A}</number></p><p><operator>-</operator><number>${bPlusC}</number></p><hr><p><number>${result}</number></p>`;
        answerElementHtml += "</answer-option>";
      }
      function doneASubstractCSubstractB() {
        ...
        answerElementHtml += `<answer-option edu-chars="${charsStr}">`;
        answerElementHtml +=
          `${debitHtmlStep1}<p><number>${A}</number></p><p><operator>-</operator><number>${C}</number></p><hr>`;
        answerElementHtml +=
          `${debitHtmlStep2}<p><number>${aSubstractC}</number></p><p><operator>-</operator><number>${B}</number></p><hr><p><number>${result}</number></p>`;
        answerElementHtml += "</answer-option>";
      }
      function doneASubstractBSubstractC() {
        ...
        answerElementHtml +=
          `${debitHtmlStep1}<p><number>${A}</number></p><p><operator>-</operator><number>${C}</number></p><hr>`;
        answerElementHtml +=
          `${debitHtmlStep2}<p><number>${aSubstractB}</number></p><p><operator>-</operator><number>${B}</number></p><hr><p><number>${result}</number></p>`;
        answerElementHtml += "</answer-option>";
      }
      if (withBrackets) {
        // 算法1：连续两式：B+C、A-(B+C)
        ...
        answerElementHtml += `<answer-option edu-chars="${charsStr}">`;
        answerElementHtml +=
          `${debitHtml1}<p><number>${A}</number></p><p><operator>-</operator><number>${B}</number></p><hr><p><number>${aSubstractB}</number></p>`;
        answerElementHtml += '<hr class="step"/>';
        answerElementHtml +=
    ...
    this.fillElementArrayOfASubtractBThenPlusC = (
      ...
      function doneAPlusCSubstractB() {
        const tensCarryHtml = aDigits + cDigits > 9
          ? '<carry edu-digit="tens">1</carry>'
          : "";
        const hundredsCarryHtml = aLastTowDigits + cLastTowDigits > 99
          ? '<carry edu-digit="hundreds">1</carry>'
          : "";
        ...
        answerElementHtml +=
          `<p><number>${A}</number></p><p><operator>+</operator><number>${C}</number>${hundredsCarryHtml}${tensCarryHtml}</p><hr>`;
        answerElementHtml +=
          `${debitHtmlAPlusCSubstractB}<p><number>${aPlusC}</number></p><p><operator>-</operator><number>${B}</number></p><hr><p><number>${result}</number></p>`;
        answerElementHtml += `</answer-option>`;
      }
      if (withBrackets) {
        ...
        answerElementHtml += `<answer-option edu-chars="${charsStr}">`;
        answerElementHtml +=
          `${debitHtmlStep1}<p><number>${B}</number></p><p><operator>-</operator><number>${C}</number></p><hr><p><number>${bSubstractC}</number></p>`;
        answerElementHtml += '<hr class="step"/><p></p>';
        answerElementHtml +=
          `${debitHtmlStep2}<p><number>${A}</number></p><p><operator>-</operator><number>${bSubstractC}</number></p><hr><p><number>${result}</number></p>`;
        answerElementHtml += `</answer-option>`;
        if (bDigits !== cDigits) {
          if (A >= B && aDigits === bDigits) {
            ...
            answerElementHtml += `<answer-option edu-chars="${charsStr}">`;
            answerElementHtml +=
              `${debitHtmlASubstractB}<p><number>${A}</number></p><p><operator>-</operator><number>${B}</number></p><hr><p><number>${aSubstractB}</number></p>`;
        ...
        // 算法2：两式合并
        const debitExtendHtmlASubstractB = `${
          getDebitHtml(A, B)
        ...
        if (bDigits === cDigits && aDigits !== cDigits) {
          ...
          answerElementHtml +=
            `<answer-option edu-chars="${charsStr}"><p></p><p></p>`;
          answerElementHtml +=
            `${debitHtml}<p><number>${B}</number></p><p><operator>-</operator><number>${C}</number></p><hr>`;
    ...
    this.fillElementArrayOfAPlusBThenSubtractC = (
      ...
      function doneASubstractCPlusB() {
        ...
        const bLastTowDigits = B % 100;
        const tensCarryHtml = acDigits + bDigits > 9
          ? '<carry edu-digit="tens">1</carry>'
          : "";
        const hundredsCarryHtml = acLastTowDigits + bLastTowDigits > 99
          ? '<carry edu-digit="hundreds">1</carry>'
          : "";
        answerElementHtml += `<answer-option edu-chars="${charsStr}">`;
        answerElementHtml +=
          `${debitHtml}<p><number>${A}</number></p><p><operator>-</operator><number>${C}</number></p><hr>`;
        answerElementHtml +=
          `<p><number>${AC}</number></p><p><operator>+</operator><number>${B}</number>${hundredsCarryHtml}${tensCarryHtml}</p><hr><p><number>${result}</number></p>`;
        answerElementHtml += `</answer-option>`;
      }
      function doneAPlusBSubstractC(twoOption = false) {
        ...
        if (twoOption) {
          ...
          answerElementHtml += '<hr class="step"/><p></p>';
          answerElementHtml +=
            `${debitHtmlABSubstractC}<p><number>${AB}</number></p><p><operator>-</operator><number>${C}</number></p><hr><p><number>${result}</number></p>`;
          answerElementHtml += `</answer-option>`;
        }
        const tensCarryHtml = aDigits + bDigits > 9
          ? '<carry edu-digit="tens">1</carry>'
          : "";
        const hundredsCarryHtml = aLastTowDigits + bLastTowDigits > 99
          ? '<carry edu-digit="hundreds">1</carry>'
          : "";
        answerElementHtml += `<answer-option edu-chars="${charsStr}">`;
        answerElementHtml +=
          `<p><number>${A}</number></p><p><operator>+</operator><number>${B}</number>${hundredsCarryHtml}${tensCarryHtml}</p><hr>`;
        answerElementHtml +=
          `${debitHtmlABSubstractC}<p><number>${AB}</number></p><p><operator>-</operator><number>${C}</number></p><hr><p><number>${result}</number></p>`;
        answerElementHtml += `</answer-option>`;
      }
      function doneBSubstractCThenPlusA(twoOption = false) {
        ...
        const cLastTowDigits = C % 100;
        const tensCarryHtml = bcDigits + cDigits > 9
          ? '<carry edu-digit="tens">1</carry>'
          : "";
        const hundredsCarryHtml = bcLastTowDigits + cLastTowDigits > 99
          ? '<carry edu-digit="hundreds">1</carry>'
          : "";
        if (twoOption) {
          answerElementHtml += `<answer-option edu-chars="${charsStr}">`;
          answerElementHtml +=
            `${debitHtml}<p><number>${B}</number></p><p><operator>-</operator><number>${C}</number></p><hr><p><number>${BC}</number></p>`;
          answerElementHtml += '<hr class="step"/><p></p>';
          answerElementHtml +=
            `<p><number>${BC}</number></p><p><operator>+</operator><number>${A}</number>${hundredsCarryHtml}${tensCarryHtml}</p><hr><p><number>${result}</number></p>`;
          answerElementHtml += `</answer-option>`;
        }
        answerElementHtml += `<answer-option edu-chars="${charsStr}">`;
        answerElementHtml +=
    ...
    this.appendReportElements = (info, questionRowsArray, answerRowsArray) => {
      const {
        data: {
          onlyMentalArithmetic, // innerLineStyle,
          // outerLineStyle,
        },
      } = this;
    ...
    this.pageSubjectFontSizeElement = createElement("input");
    this.questionFontSizeElement = createElement("input");
    this.onlyMentalArithmeticRadioArray = [];
    ...
    // };
    this.yearElement = createElement("input");
    this.monthElement = createElement("input");
    this.dayElement = createElement("input");
    /** OK
     *
     * @param wrapElement
     */
  ...
  fillElementArrayOfAPlusB(
    ...
    if (!onlyMentalArithmetic) {
      const charsStr = (result.toString().length + 1).toString();
      answerElementHtml +=
        `${this.questionElementHtmlAppendStart4}<answer-option edu-chars="${charsStr}"><p></p>${
  ...
  getHtmlOfAPlusB(A, B, result) {
    const tensCarryHtml = A % 10 + B % 10 > 9
      ? '<carry edu-digit="tens">1</carry>'
      : "";
    const hundredsCarryHtml = A % 100 + B % 100 > 99
      ? '<carry edu-digit="hundreds">1</carry>'
      : "";
  ...
  getHtmlOfAMultiplyB(A, B, result) {
    ...
    for (let i = bLength; i > 0; --i) {
      ...
      for (let charLoop = productCharCount; charLoop > 0; --charLoop) {
        ...
        // 3-1-2+2=2
        const arrayIndex = resultLength - 1 - (bLength - i) -
  ...
  fillElementArrayOfASubtractB(
    ...
    let answerElementHtml = `<p>${
      commonHtml.replace(/ /g, "<i> </i>")
    }${result}</p>`;
    if (!onlyMentalArithmetic) {
      const debitHtml = getDebitHtml(A, B);
      answerElementHtml +=
        `${this.questionElementHtmlAppendStart4}<answer-option edu-chars="${charsStr}">${debitHtml}<p><number>${A}</number></p><p><operator>-</operator><number>${B}</number></p><hr><p><number>${result}</number></p></answer-option></div>`;
    }
    this.fillElementListCore(
      questionElementHtml,
      answerElementHtml,
  ...
  getDebitHtml(A, B) {
    ...
    const digitDebitHtml = tensNeedDebit
      ? `<debit edu-digit="digit"><debit-number>${10 +
        aDigits}</debit-number></debit>`
      : "";
    const tensDebitHtml = tensNeedDebit || hundredsNeedDebit
      ? `<debit edu-digit="tens"><debit-number>${(hundredsNeedDebit ? 10 : 0) +
        Math.floor(aHundreds / 10) -
        (tensNeedDebit ? 1 : 0)}</debit-number>${
        tensNeedDebit ? "<debit-circle>·</debit-circle>" : ""
      }</debit>`
      : "";
    const hundredsDebitHtml = hundredsNeedDebit
      ? `<debit edu-digit="hundreds"><debit-number>${(thousandsNeedDebit
        ? 10
        : 0) +
        Math.floor(A % 1000 / 100) -
        (hundredsNeedDebit
          ? 1
          : 0)}</debit-number><debit-circle>·</debit-circle></debit>`
      : "";
    // return debitHtml;
    ...
    for (let rowIndex = 0; rowIndex < rows; ++rowIndex) {
      ...
      for (
        ...
        switch ((isPlus ? 0 : 2) + (has ? 0 : 1)) {
          ...
          case 3:
          default:
            ({ A, B, aMultiplyB } = aMultiplyBMaybeNotDebitMinusArray[
        ...
        const result = switchOrder
          ? A * B * (isPlus ? 1 : -1) + C
          : A * B + C * (isPlus ? 1 : -1);
        const charsStr = ((isPlus ? result : aMultiplyB).toString().length + 1)
          .toString();
        // switchOrder:
        // true => A±B×C=D
        // false => A×B±C=D
        ...
        const questionElementHtml =
          `<p>${commonHtml}</p>${questionElementHtmlAppend}`;
        let answerElementHtml = `<p>${commonHtml}${result}</p>`;
        if (!onlyMentalArithmetic) {
          answerElementHtml += `<div edu-flex="8">`;
          answerElementHtml += `<answer-option edu-chars="${charsStr}">`;
    ...
    for (let rowIndex = 0; rowIndex < rows; ++rowIndex) {
      ...
      for (
        ...
        if (!onlyMentalArithmetic) {
          ...
          if (A < 10) {
            answerElementHtml +=
              `<p><operator>×</operator><number>${A}</number></p><hr><p><number>${result}</number></p>`;
          } else {
            answerElementHtml += '<hr class="step"/>';
            answerElementHtml += this.getHtmlOfAMultiplyB(B_C, A, result);
          }
  ...
  countByArithmetic11(info, questionRowsArray, answerRowsArray) {
    ...
    for (let rowIndex = 0; rowIndex < rows; ++rowIndex) {
      ...
      for (
        ...
        answerElementHtml = `<p>${
          html.replace(/ /g, "<i> </i>")
        }${answerElementHtml}</p>`; // `${questionElementHtml}${answerElementHtml}`;
        this.fillElementListCore(
          questionElementHtml,
          answerElementHtml,
          questionElementArray,
        ...
        if (scope === "B" && getIntegerRandom(1, 10) < 2) {
          ...
          switch (getIntegerRandom(1, 4)) {
            case 1:
              ...
              D = getIntegerRandom(1, 9);
              E = getIntegerRandom(1, 9);
              answerElementHtml = [A, B].sort((prev, next) => prev - next).join(
                " ",
              ); // `${A} ${B}`;
              break;
            case 2:
              ...
              D = getIntegerRandom(1, 9);
              E = getIntegerRandom(1, 9);
              answerElementHtml = [A, B, C].sort((prev, next) => prev - next)
                .join(" "); // `${A} ${B} ${C}`;
              break;
            case 3:
              while (true) {
              ...
              D = 10 - ((A + B + C) % 10);
              E = getIntegerRandom(1, 9);
              answerElementHtml = [A, B, C, D].sort((prev, next) => prev - next)
                .join(" "); // `${A} ${B} ${C} ${D}`;
              break;
            case 4:
            default:
              ...
              E = 10 - ((A + B + C + D) % 10);
              answerElementHtml = [A, B, C, D, E].sort((prev, next) =>
                prev - next
              ).join(" "); // `${A} ${B} ${C} ${D} ${E}`;
              break;
          }
