...
var BrickCore = /** @class */ (function (_super) {
    ...
    function BrickCore() {
        ...
        _this.countHollowOutSvg = function (svg, item, PAPER_WIDTH, PAPER_HEIGHT, HOLLOW_STYLE) {
            ...
            // kind,
            // rowCount,
            innerLineStyle = item.innerLineStyle, outerLineStyle = item.outerLineStyle, cutLineStyle = item.cutLineStyle;
        ...
        _this.countInterlacedHexagonSvg = function (svg, item, PAPER_WIDTH, PAPER_HEIGHT) {
            ...
            var SIDE_LENGTH = item.length, 
            // kind,
            rowCount = item.rowCount, innerLineStyle = item.innerLineStyle, outerLineStyle = item.outerLineStyle, cutLineStyle = item.cutLineStyle;
            var ANGLE30 = (Math.PI * 30) / 180;
            var ANGLE60 = (Math.PI * 60) / 180;
            var SIN30 = Math.sin(ANGLE30);
            var SIN60 = Math.sin(ANGLE60);
        ...
        _this.countPentagonSvg = function (svg, item, PAPER_WIDTH, PAPER_HEIGHT) {
            ...
            var SIDE_LENGTH = item.length, 
            // kind,
            rowCount = item.rowCount, innerLineStyle = item.innerLineStyle, outerLineStyle = item.outerLineStyle;
        ...
        _this.countUnifySvg = function (svg, item, PAPER_WIDTH, PAPER_HEIGHT) {
            ...
            var SIDE_LENGTH = item.length, 
            // kind,
            rowCount = item.rowCount, INNER_LINE_STYLE = item.innerLineStyle, OUTER_LINE_STYLE = item.outerLineStyle;
            ...
            for (var i = 0; i < 5; ++i) {
                ...
                appendPentagon(svg, SIDE_LENGTH, INNER_LINE_STYLE, OUTER_LINE_STYLE, HEXAGON_Y3, leftOfBelow + SIN30_MULTIPLY_SIDE_LENGTH, i < 4 ? 0 : 1, false, viewBox); // OK
                if (i === 4) {
                    appendPentagon(svg, SIDE_LENGTH, INNER_LINE_STYLE, OUTER_LINE_STYLE, TOP, left + SIN30_MULTIPLY_SIDE_LENGTH, 2, false, viewBox); // OK
