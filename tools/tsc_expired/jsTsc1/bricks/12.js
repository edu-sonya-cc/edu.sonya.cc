...
var BrickCore = /** @class */ (function (_super) {
  ...
  function BrickCore() {
    ...
    _this.countDataAndComputedData = function () {
      ...
      list.forEach(function (item) {
        return item.length = formatCentile(item.length);
      });
      var elementList = [];
      ...
      list.filter(function (_a) {
        ...
        return _this.getLatticeOutSvgList(item, MAX_X, MAX_Y, mmToPxScale)
      ...
      var getAutomaticPaginationHtmlFromChildList =
        _this.getAutomaticPaginationHtmlFromChildList;
      ...
      var latticeHtml = getAutomaticPaginationHtmlFromChildList(
        elementList.splice(0, elementList.length / 2),
        MAX_X,
        MAX_Y,
      );
      // console.log('after append half, ', elementList.length);
      var latticeHtmlMirror = getAutomaticPaginationHtmlFromChildList(
        elementList,
        MAX_X,
        MAX_Y,
      )
      ...
      list.filter(function (_a) {
        ...
        // 	elementList.push(element);
        // })
        return _this.getPokerSvgList(item, MAX_X, MAX_Y, mmToPxScale).forEach(
          function (element) {
            elementList.push(element);
          },
        );
      });
      var pokerHtml = getAutomaticPaginationHtmlFromChildList(
        elementList,
        MAX_X,
        MAX_Y,
      );
      ...
      var en = "".concat(FILENAME_POSTFIX, "multiplicationtable");
      var zh_cn = "".concat(FILENAME_POSTFIX, "\u4E58\u6CD5\u53E3\u8BC0\u8868");
      var zh_tw = "".concat(FILENAME_POSTFIX, "\u4E58\u6CD5\u53E3\u8A23\u8868");
    ...
    _this.getElementList = function (
      ...
      switch (item.kind) {
        case "vertical":
        case "horizontal":
          return _this.getLatticeOutSvgList(
            item,
            PAPER_WIDTH,
            PAPER_HEIGHT,
            mmToPxScale,
     ...
     */
    _this.getPokerSvgList = function (
      item,
      PAPER_WIDTH,
      PAPER_HEIGHT,
      mmToPxScale,
      ...
      var _a = svgSpace.edu.sonya.cc.SvgHelper,
        ...
        createSvgPath = _a.createSvgPath,
      ...
      var width = item.length,
        scope = item.scope,
        kind = item.kind,
        pokerIncludeZero = item.pokerIncludeZero,
        copies = item.copies,
      ...
      var indiaChars = westChars.concat([
        "十三",
        "十四",
        "十五",
        "十六",
      ...
      for (var i = MIN; i <= MAX; ++i) {
        ...
        for (var j = isFull ? MIN : i; j <= MAX; ++j) {
          ...
          if (isChinese) {
            ...
            if (product < 10) {
              productStr = "\u5F97<br/>".concat(onesPlaceStr);
            } else if (product === 10) {
              productStr = "一<br/>十";
            } else if (product < 20) {
              productStr = "\u5341<br/>".concat(onesPlaceStr);
            } else {
              if (
                scope === "chinese" || (product % 100 === 0 || product < 100)
              ) {
                if (product % 100 === 0) {
                  productStr = "".concat(chars[hundreds], "<br/>\u767E");
                } else {
                  productStr += "".concat(chars[tens], "<br/>\u5341");
                  if (onesPlace > 0) {
                    productStr += "<br/>".concat(onesPlaceStr);
                  }
                }
              } else {
                productStr = hundreds > 0
                  ? "".concat(chars[hundreds], "\u767E<br/>")
                  : "";
                if (hundreds && tens === 0 && onesPlace) {
                  productStr += "零";
                }
                productStr += tens > 0
                  ? "".concat(chars[tens], "\u5341").concat(
                    onesPlace > 0 ? "<br/>" : "",
                  )
                  : "";
            ...
            jStr = "\u00D7".concat(j); // j.toString();
            productStr = "\uFF1D".concat(product); // product.toString();
          }
          contents.push(iStr);
          contents.push(jStr);
      ...
      // 	}
      // }
      var COUNT_PER_PAGE = Math.floor(PAPER_WIDTH / width) *
        Math.floor(PAPER_HEIGHT / height);
      var pokerCountHasNumber = contents.length * copies;
      var totalCount = Math.ceil(pokerCountHasNumber / COUNT_PER_PAGE) *
        COUNT_PER_PAGE;
     ...
     * @param mmToPxScale 毫米到像素折算比例
     */
    _this.getLatticeOutSvgList = function (
      item,
      PAPER_WIDTH,
      PAPER_HEIGHT,
      mmToPxScale,
      ...
      if (NORMAL_NUMBERS) {
        while (true) {
          ...
          _this.appendNormalNumbersToPage(
            SIDE_LENGTH,
            firstItem,
            currentPage,
            innerLineStyle,
      ...
      var cssHeight = "".concat(SIDE_LENGTH * ROW_COUNT, "mm");
      ...
      pages
        .forEach(function (page) {
          var mainWrapper = page.mainWrapper,
            mainWrapperColCount = page.mainWrapperColCount,
            otherWrapper = page.otherWrapper,
            mainWrapperMirror = page.mainWrapperMirror,
            otherWrapperMirror = page.otherWrapperMirror;
          // 强制补全整页
          var otherWrapperColCount = COL_COUNT - mainWrapperColCount;
          page.otherWrapperColCount = otherWrapperColCount;
          // flex-direction:column;
          var mainWrapperCss = "width:".concat(
            SIDE_LENGTH * mainWrapperColCount,
            "mm;height:",
          ).concat(cssHeight, ";").concat(cssFlex);
          ...
          if (otherWrapperColCount) {
            // flex-direction:column;
            var otherWrapperCss = "width:".concat(
              SIDE_LENGTH * otherWrapperColCount,
              "mm;height:",
            ).concat(cssHeight, ";").concat(cssFlex);
      ...
      pages.forEach(function (_a) {
        var mainWrapperMirror = _a.mainWrapperMirror,
          otherWrapperMirror = _a.otherWrapperMirror,
          otherWrapperColCount = _a.otherWrapperColCount;
    ...
    _this.appendOtherNumbers = function (
      SIDE_LENGTH,
      ROW_COUNT,
      OTHER_NUMBERS,
      currentPage,
      ...
      var list = [];
      OTHER_NUMBERS.forEach(function (item) {
        return list.push(item);
      });
      list.sort(function (prev, next) {
        return (next.rowCount * 100 + next.digit) -
          (prev.rowCount * 100 + prev.digit);
      });
      var colCountArray = [];
      ...
      while (remainingColCount > 0) {
        ...
        var _loop_2 = function (i) {
          ...
          if (filterResult.length) {
            ...
            var mmSubWrapperWidth = "".concat(subWrapperWidth, "mm");
            var mmSubWrapperHeight = "".concat(subWrapperHeight, "mm");
            var subWrapperCss =
              "display:flex;flex-wrap:wrap;flex-direction:column;width:".concat(
                subWrapperWidth,
                "mm;height:",
              ).concat(subWrapperHeight, "mm;");
            ...
            _this.fillHorizontalSubWrapper(
              subWrapperRemainRow,
              searchMax,
              list,
              OTHER_NUMBERS,
    ...
    _this.appendOtherNumberItem = function (
      SIDE_LENGTH,
      item,
      subWrapper,
      subWrapperMirror,
      ...
      var HEIGHT_PX = formatMillimeter(height * mmToPxScale);
      var HEIGHT_SUBTRACT_DIAMETER_PX = HEIGHT_PX - DIAMETER_PX;
      var mmSvgWidth = "".concat(width, "mm");
      var mmSvgHeight = "".concat(height, "mm");
      ...
      var mmSubSvgWidthOrHeight = "".concat(SIDE_LENGTH, "mm");
      for (var colIndex = 0; colIndex < trueColCount; ++colIndex) {
        var mmX = "".concat(SIDE_LENGTH * colIndex, "mm");
        for (var rowIndex = 0; rowIndex < digit; ++rowIndex) {
          var mmY = "".concat(
            height - SIDE_LENGTH * rowIndex - SIDE_LENGTH,
            "mm",
          );
    ...
    _this.appendNormalNumbersToPage = function (
      SIDE_LENGTH,
      _a,
      currentPage,
      innerLineStyle,
      ...
      var _b = svgSpace.edu.sonya.cc.SvgHelper,
        ...
        createSvgPath = _b.createSvgPath,
      ...
      var DIAMETER_PX = RADIUS_PX * 2;
      var WIDTH_SUBTRACT_DIAMETER_PX = WIDTH_PX - DIAMETER_PX;
      var mainWrapper = currentPage.mainWrapper,
        mainWrapperMirror = currentPage.mainWrapperMirror;
      ...
      series.forEach(function (rowCount) {
        ...
        var mmWidth = "".concat(width, "mm");
        var mmHeight = "".concat(height, "mm");
    ...
    _this.createTableBodyRow = function (item) {
      var _a = item,
        ...
        scope = _a.scope,
        kind = _a.kind,
        pokerIncludeZero = _a.pokerIncludeZero,
        copies = _a.copies,
  ...
  BrickCore.prototype.fillHorizontalSubWrapper = function (
    subWrapperRemainRow,
    maxColCount,
    list,
    OTHER_NUMBERS,
  ...
  BrickCore.prototype.appendSvgPath = function (
    createSvgPath,
    stroke,
    RADIUS_PX,
    WIDTH_SUBTRACT_DIAMETER_PX,
    ...
    path.setAttribute(
      "d",
      "M 0, ".concat(RADIUS_PX)
        .concat(
          " a ".concat(RADIUS_PX, " ").concat(RADIUS_PX, " 0 0 1 ").concat(
            RADIUS_PX,
            " -",
          ).concat(RADIUS_PX),
          " h ".concat(WIDTH_SUBTRACT_DIAMETER_PX, " "),
          " a ".concat(RADIUS_PX, " ").concat(RADIUS_PX, " 0 0 1 ").concat(
            RADIUS_PX,
            " ",
          ).concat(RADIUS_PX),
          " v ".concat(HEIGHT_SUBTRACT_DIAMETER_PX, " "),
          " a ".concat(RADIUS_PX, " ").concat(RADIUS_PX, " 0 0 1 -").concat(
            RADIUS_PX,
            " ",
          ).concat(RADIUS_PX),
  ...
  BrickCore.prototype.fillIndiaNumberSeries = function (
    ROW_COUNT,
    NUMBER_SERIES,
    OTHER_NUMBER_SERIES,
  ) {
    switch (ROW_COUNT) {
      case 29:
        ...
        [6, 4, 3, 2, 1].forEach(function (number) {
          return OTHER_NUMBER_SERIES.push(number);
        });
        break;
      case 28:
      ...
      case 22:
        ...
        [11, 2, 1].forEach(function (number) {
          return OTHER_NUMBER_SERIES.push(number);
        });
        break;
      case 21:
  ...
  BrickCore.prototype.fillWestNumberSeries = function (
    ROW_COUNT,
    NUMBER_SERIES,
    OTHER_NUMBER_SERIES,
  ) {
    switch (ROW_COUNT) {
      ...
      case 17:
        ...
        [4, 3, 2, 1].forEach(function (number) {
          return OTHER_NUMBER_SERIES.push(number);
        });
        break;
      case 16:
  ...
  BrickCore.prototype.fillChineseNumberSeries = function (
    ROW_COUNT,
    NUMBER_SERIES,
    OTHER_NUMBER_SERIES,
  ) {
    switch (ROW_COUNT) {
      ...
      case 21:
        ...
        NUMBER_SERIES.push([8, 7, 6]);
        [5, 4, 3, 1].forEach(function (number) {
          return OTHER_NUMBER_SERIES.push(number);
        });
        break;
      case 20:
