...
abstract class PokerBase extends BrickBase {
  ...
  protected countDataAndComputedData = (): void => {
    ...
    * { box-sizing:border-box; }
    ...
    cell {width:${CARD_WIDTH}mm;height:${CARD_HEIGHT}mm;border-radius:${
      (CARD_WIDTH * 3) / 40
  ...
  protected abstract countPokerDataAndComputedData(pokerKind: number, countPerPage: number);
  ...
  private pokerWidthElement = createElement('input') as HTMLInputElement;
  private initPokerWidthElements = (wrapElement: HTMLDivElement): void => {
    const {
      data: { pokerWidth },
      pokerWidthElement,
    } = this;
  ...
  private pokerHeightElement = createElement('input') as HTMLInputElement;
  private initPokerHeightElements = (wrapElement: HTMLDivElement): void => {
    const {
      data: { pokerHeight },
      pokerHeightElement,
    } = this;
  ...
  private fontSizeElement = createElement('input') as HTMLInputElement;
  private initFontSizeElements = (wrapElement: HTMLDivElement): void => {
    const {
      data: { fontSize },
      fontSizeElement,
    } = this;
  ...
  private backFontSizeElement = createElement('input') as HTMLInputElement;
  private initBackFontSizeElements = (wrapElement: HTMLDivElement): void => {
    const {
      data: { backFontSize },
      backFontSizeElement,
    } = this;
  ...
  private appendPokerSizeButtons = (wrapElement: HTMLDivElement): void => {
    [
      {
        i18n: {
          en: 'Small',
      ...
      const buttonElement = createElement('button') as HTMLButtonElement;
      ...
      buttonElement.onclick = (event: Event) => {
        const widthValue = parseInt(buttonElement.getAttribute('edu-to-width') as string, 0);
        const heightValue = parseInt(buttonElement.getAttribute('edu-to-height') as string, 0);
        this.data.pokerWidth = widthValue;
        this.data.pokerHeight = heightValue;

        (this.pokerWidthElement as HTMLInputElement).value = widthValue.toString();
        (this.pokerHeightElement as HTMLInputElement).value = heightValue.toString();
        this.saveConfigAndBuildIfAllowed();
  ...
  protected updateOtherData = (newData: any): void => {
    ...
    pokerKindElementArray.forEach((element: HTMLInputElement) => {
