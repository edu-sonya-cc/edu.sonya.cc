...
var BrickCore = /** @class */ (function (_super) {
    ...
    function BrickCore() {
        ...
        _this.countDataAndComputedData = function () {
            ...
            pageSubjectFontSize = _b.pageSubjectFontSize, questionFontSize = _b.questionFontSize;
        ...
        _this.fillElementArrayOfAPlusBThenC = function (has, questionElementHtmlAppend, onlyMentalArithmetic, questionElementArray, answerElementArray, withBrackets) {
            ...
            var questionElementHtml = "<p>" + commonHtml + "</p>" + questionElementHtmlAppend; // .replace(/~charCount~/g, charsStr)
        ...
        _this.fillElementArrayOfASubtractBThenC = function (has, questionElementHtmlAppend, onlyMentalArithmetic, questionElementArray, answerElementArray, withBrackets) {
            ...
            var commonHtml = withBrackets
                ? A + "<i> </i>-<i> </i>(" + B + "<i> </i>+<i> </i>" + C + ")<i> </i>=<i> </i>"
                : A + "<i> </i>-<i> </i>" + B + "<i> </i>-<i> </i>" + C + "<i> </i>=<i> </i>";
            var questionElementHtml = "<p>" + commonHtml + "</p>" + questionElementHtmlAppend; // .replace(/8/g, charsStr)
            var answerElementHtml = "<p>" + commonHtml + result + "</p>";
            if (onlyMentalArithmetic) {
                _this.fillElementListCore(questionElementHtml, answerElementHtml, questionElementArray, answerElementArray);
                return;
         ...
         */
        _this.fillElementList = function (onlyMentalArithmetic, item, 
        // kind: string,
        // countPerRow: number,
        questionElementArray, answerElementArray) {
    ...
    BrickCore.prototype.fillElementArrayOfASubtractB = function (has, min, max, questionElementArray, answerElementArray) {
        ...
        var result = A - B;
        var commonHtml = A + " - " + B + " = ";
        var questionElementHtml = "<p>" + commonHtml.replace(/ /g, '<i> </i>') + "</p>" + questionElementHtmlAppend; // .replace(/4/g, charsStr)
    ...
    BrickCore.prototype.countByArithmetic1 = function (info, questionRowsArray, answerRowsArray) {
        var getIntegerRandom = this.getIntegerRandom;
        var kind = info.kind, catalog = info.catalog, scope = info.scope, rows = info.rows, 
        // independentPagination,
        textStyle = info.textStyle, countPerRow = info.countPerRow, rowsOccupied = info.rowsOccupied, rowCountPerPage = info.rowCountPerPage;
    ...
    BrickCore.prototype.countByArithmetic6Or7 = function (info, questionRowsArray, answerRowsArray, withBrackets) {
        ...
        // kind,
        // catalog,
        scope = info.scope, rows = info.rows, 
        // independentPagination,
        textStyle = info.textStyle, countPerRow = info.countPerRow, rowsOccupied = info.rowsOccupied, rowCountPerPage = info.rowCountPerPage;
        // console.log('countByArithmetic11', rowsOccupied);
        var rowCountPerPageStr = rowCountPerPage.toString();
                ...
                if (scope === 'B' && getIntegerRandom(1, 10) < 2) {
                    ...
                    switch (getIntegerRandom(1, 4)) {
                        case 1:
                            ...
                            D = getIntegerRandom(1, 9);
                            E = getIntegerRandom(1, 9);
                            answerElementHtml = [A, B].sort(function (prev, next) { return prev - next; }).join(' '); // `${A} ${B}`;
                            break;
                        case 2:
                            while (true) {
                                B = getIntegerRandom(1, 9);
                            ...
                            D = getIntegerRandom(1, 9);
                            E = getIntegerRandom(1, 9);
                            answerElementHtml = [A, B, C].sort(function (prev, next) { return prev - next; }).join(' '); // `${A} ${B} ${C}`;
                            break;
                        case 3:
                            while (true) {
                                B = getIntegerRandom(1, 9);
                            ...
                            D = 10 - ((A + B + C) % 10);
                            E = getIntegerRandom(1, 9);
                            answerElementHtml = [A, B, C, D].sort(function (prev, next) { return prev - next; }).join(' '); // `${A} ${B} ${C} ${D}`;
                            break;
                        case 4:
                        default:
                            while (true) {
                            ...
                            E = 10 - ((A + B + C + D) % 10);
                            answerElementHtml = [A, B, C, D, E].sort(function (prev, next) { return prev - next; }).join(' '); // `${A} ${B} ${C} ${D} ${E}`;
                            break;
                    }
                }
    ...
    // kind: string,
    // countPerRow: number,
    question, answer, questionElementArray, answerElementArray) {
     ...
     * @param answerRowsArray
     */
    BrickCore.prototype.fillExhaustibleList = function (exhaustibleItem, info, onlyMentalArithmetic, 
    // kind: string,
    // countPerRow: number,
    questionRowsArray, answerRowsArray) {
        var getIntegerRandom = this.getIntegerRandom;
        var 
        // kind,
        catalog = info.catalog, 
        // scope,
        rows = info.rows, countPerRow = info.countPerRow, rowsOccupied = info.rowsOccupied, rowCountPerPage = info.rowCountPerPage, 
        // independentPagination,
    ...
    BrickCore.prototype.addCommonItem = function (info, kindArray, buttonList) {
        var 
        // kind,
        catalog = info.catalog, scope = info.scope, rows = info.rows, independentPagination = info.independentPagination, textStyle = info.textStyle, countPerRow = info.countPerRow, rowsOccupied = info.rowsOccupied, rowCountPerPage = info.rowCountPerPage;
