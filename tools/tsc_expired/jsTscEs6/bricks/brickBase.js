...
export class BrickBase {
  constructor(appendData, otherComputedData, newPageSizeArray = ["A3", "A4"]) {
    ...
    this.getCss = () => this.computedData.css;
    this.getHtml = () => this.computedData.html;
    this.data = {
      paperSize: "A4",
      isLandscape: false,
      maxX: 200,
    ...
    this.getWrapElement = (strongI18n) => {
    ...
    this.getAutomaticPaginationHtmlFromChildList = (
      list,
      MAX_X,
      MAX_Y,
      pageClass = "",
    ...
    this.appendAutomaticPaginationControls = (wrapper, list, MAX_X, MAX_Y) => {
      if (list.length === 0) {
        return;
      }
    ...
    // }
    this.formatDecile = (length) => Math.round(length * 10) / 10;
    this.formatCentile = (length) => Math.round(length * 100) / 100;
    this.formatMillimeter = (length) => Math.round(length * 1000) / 1000;
    this.data = Object.assign(Object.assign({}, this.data), appendData);
    this.DEFAULT_DATA_JSON = JSON.stringify(this.data);
    this.computedData = Object.assign(
      Object.assign({}, this.computedData),
      otherComputedData,
    );
