...
class BrickCore extends BrickWithTableBase {
  ...
  constructor() {
    super({}, {});
    this.idOrClassPrefix = "brickPageFootball";
    /** OK
    ...
    this.countDataAndComputedData = () => {
      ...
      html += this.getAutomaticPaginationHtmlFromChildList(
        elementList,
        MAX_X,
        MAX_Y,
      );
     ...
     */
    this.getElement = (item, mmToPxScale, PAPER_WIDTH, PAPER_HEIGHT) => {
    ...
    this.countHollowOutSvg = (
      ...
      const ROW_COUNT =
        Math.floor((Math.floor(PAPER_HEIGHT / ROW_HEIGHT)) / 9) * 9;
      ...
      for (let rowIndex = 0; rowIndex < ROW_COUNT; ++rowIndex) {
        const rowGroupIndex = rowIndex === 0
          ? 0
          : Math.floor((rowIndex - 1) / 8);
        ...
        for (let groupIndex = 0; groupIndex < COL_GROUP_COUNT; ++groupIndex) {
          const LEFT_GROUP = SIDE_LENGTH * SIN30 +
            (rowIndex % 2 === 0
              ? 0
      ...
      const widthMm = WIDTH_OF_ONE * 6 * COL_GROUP_COUNT +
    ...
    this.countInterlacedHexagonSvg = (svg, item, PAPER_WIDTH, PAPER_HEIGHT) => {
      ...
      for (let rowIndex = 0; rowIndex < ROW_COUNT; ++rowIndex) {
        const TOP = ROW_HEIGHT * rowIndex;
        for (let colIndex = 0; colIndex < COL_COUNT; ++colIndex) {
          const LEFT = SIDE_LENGTH * SIN30 + WIDTH_OF_ONE * colIndex +
      ...
      const widthMm = WIDTH_OF_ONE * COL_COUNT + SIN30_MULTIPLY_SIDE_LENGTH +
        SIDE_LENGTH * 0.5;
    ...
    this.countPentagonSvg = (svg, item, PAPER_WIDTH, PAPER_HEIGHT) => {
      ...
      const ROW_HEIGHT = SIN72_MULTIPLY_LONG_SIDE_LENGTH +
    ...
    this.appendPentagon = (
      ...
      const PENTAGON_WIDTH = PENTAGON_CORE_WIDTH +
    ...
    this.countUnifySvg = (svg, item, PAPER_WIDTH, PAPER_HEIGHT) => {
      ...
      const PENTAGON_FULL_WIDTH = PENTAGON_WIDTH +
        PENTAGON_WIDTH * OUTSIDE_SCALE;
      ...
      for (let i = 0; i < 5; ++i) {
        ...
        if (!skipBelowLines) {
          ...
          appendLine(svg, OUTER_LINE_STYLE, X10, X1, Y10, Y1, viewBox);
          if (i === 4) {
            const X11 = X6, Y11 = Y7 - HEXAGON_OUTSIDE_Y;
