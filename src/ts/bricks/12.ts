...
class BrickCore extends BrickWithTableBase {
  protected idOrClassPrefix = 'brickPageMultiplicationTable';
  ...
  protected countDataAndComputedData = (): void => {
    ...
    const list: Array<MultiplicationTableInfo> = [];
    JSON.parse(JSON.stringify(this.data.list)).forEach((item: MultiplicationTableInfo) => {
      list.push(item);
    });
    ...
    // );
    list
      .filter(({ kind }) => latticeKindArray.indexOf(kind) > -1)
      .forEach(item =>
        this.getLatticeOutSvgList(item, MAX_X, MAX_Y, mmToPxScale).forEach(element => {
          elementList.push(element);
        }),
      );
    ...
    list
      ...
      .forEach(item =>
        ...
        this.getPokerSvgList(item, MAX_X, MAX_Y, mmToPxScale).forEach(element => {
          elementList.push(element);
        }),
      );
  ...
  private getElementList = (
    item: MultiplicationTableInfo,
    mmToPxScale: number,
    PAPER_WIDTH: number,
    PAPER_HEIGHT: number,
  ): Array<HTMLDivElement | SVGElement> => {
   ...
   */
  private getPokerSvgList = (
    item: MultiplicationTableInfo,
    PAPER_WIDTH: number,
    PAPER_HEIGHT: number,
    mmToPxScale: number,
  ): Array<HTMLDivElement | SVGElement> => {
    const list: Array<HTMLDivElement | SVGElement> = [];

    const { createSvg, createSvgPath, appendText, getTextStyleFontSizePatchCss } =
    ...
    const indiaChars = westChars.concat(['十三', '十四', '十五', '十六', '十七', '十八', '十九']);
    ...
    const CSS_COLOR_ARRAY = ['fill:#FF0000;', 'fill:#0000FF;', 'fill:#FF00FF;'];
    ...
    const fontSize =
      !isChinese || textStyle.indexOf('font-size:') === -1
        ? 0
    ...
    for (let copyLoop = 0; copyLoop < copies; ++copyLoop) {
      contents.forEach((content, index) => {
        ...
        appendText(
          svg,
          textStyle.concat(
            isChinese ? '' : getTextStyleFontSizePatchCss(parseInt(content, 0), textStyle),
  ...
  private getLatticeOutSvgList = (
    ...
    const NUMBER_SERIES: Array<Array<number>> = [];
    const OTHER_NUMBER_SERIES: Array<number> = [];
    let maxNumber = 9;
    switch (scope) {
      case 'chinese':
    ...
    const NORMAL_NUMBERS: Array<{ digit: number; series: Array<number> }> = [];
    const OTHER_NUMBERS: Array<{ digit: number; rowCount: number }> = [];
    ...
    const pages: Array<{
      ...
      mainWrapperColCount: number;
      otherWrapper: HTMLDivElement;
      otherWrapperColCount: number;
      mainWrapperMirror: HTMLDivElement;
      otherWrapperMirror: HTMLDivElement;
    }> = [];
    ...
    while (OTHER_NUMBERS.length) {
      const lastPage = {
        mainWrapper: createElement('div') as HTMLDivElement,
        mainWrapperColCount: 0, // COL_COUNT,
        otherWrapper: createElement('div') as HTMLDivElement,
        otherWrapperColCount: 0,
        mainWrapperMirror: createElement('div') as HTMLDivElement,
        otherWrapperMirror: createElement('div') as HTMLDivElement,
      };
    ...
    pages.forEach(page => {
      ...
      const mainWrapperCss = `width:${
        SIDE_LENGTH * mainWrapperColCount
      }mm;height:${cssHeight};${cssFlex}`;
  ...
  private appendOtherNumbers = (
    ...
    mmToPxScale: number,
    maxNumber: number,
    remainingColCount: number,
    isOtherWrapper: boolean,
    isVertical: boolean,
  ): void => {
    const list: Array<{
      digit: number;
    ...
    list.sort(
      (prev, next) => next.rowCount * 100 + next.digit - (prev.rowCount * 100 + prev.digit),
    );

    const colCountArray: Array<number> = [];
    ...
    while (remainingColCount > 0) {
      ...
      for (let i = 0; i < colCountCount; ++i) {
        ...
        if (filterResult.length) {
          ...
          const subWrapper = createElement('div') as HTMLDivElement;
          ...
          const subWrapperMirror = createElement('div') as HTMLDivElement;
  ...
  private fillHorizontalSubWrapper(
    subWrapperRemainRow: number,
    maxColCount: number,
    list: { digit: number; rowCount: number }[],
    OTHER_NUMBERS: { digit: number; rowCount: number }[],
    SIDE_LENGTH: number,
    subWrapper: HTMLDivElement,
    subWrapperMirror: HTMLDivElement,
    innerLineStyle: string,
    stroke: string,
    textStyle: string,
    mmToPxScale: number,
    colCountArray: Array<number>,
    isVertical: boolean,
  ) {
  ...
  private appendOtherNumberItem = (
    SIDE_LENGTH: number,
    item: { digit: number; rowCount: number },
    subWrapper: HTMLDivElement,
    subWrapperMirror: HTMLDivElement,
    innerLineStyle: string,
    stroke: string,
    ...
    const { createSvg, createSvgPath, appendLine, appendText, getTextStyleFontSizePatchCss } =
      svgSpace.edu.sonya.cc.SvgHelper;
    const { formatMillimeter } = this;

    const svg = createSvg() as SVGAElement;
    subWrapper.appendChild(svg);

    const svgMirror = createSvg() as SVGAElement;
    ...
    for (let colIndex = 0; colIndex < trueColCount; ++colIndex) {
      ...
      for (let rowIndex = 0; rowIndex < digit; ++rowIndex) {
        ...
        if (isVertical) {
          ...
          appendText(
            subSvg,
            textStyle.concat(getTextStyleFontSizePatchCss(num, textStyle), digitCss),
            num.toString(),
            textXOrY,
            textXOrY,
            0,
          ...
          appendText(
            subSvgMirror,
            textStyle.concat(getTextStyleFontSizePatchCss(numMirror, textStyle), digitCss),
            numMirror.toString(),
            textXOrY,
            textXOrY,
            0,
  ...
  private appendNormalNumbersToPage = (
    SIDE_LENGTH: number,
    { digit, series }: { digit: number; series: Array<number> },
    currentPage: {
      mainWrapper: HTMLDivElement;
      mainWrapperColCount: number;
      otherWrapper: HTMLDivElement;
    ...
    const mainWrapperSub = createElement('div') as HTMLDivElement;
    mainWrapper.appendChild(mainWrapperSub);

    const mainWrapperSubMirror = createElement('div') as HTMLDivElement;
    ...
    series.forEach(rowCount => {
      ...
      const svgMirror = createSvg() as SVGElement;
      ...
      for (let rowIndex = 0; rowIndex < rowCount; ++rowIndex) {
        ...
        const numberOffset = digit * (isVertical ? rowCount - rowIndex - 1 : rowIndex);
  ...
  protected createTableBodyRow = (item: object): void => {
    const {
      ...
      outerLineStyle,
      textStyle,
    } = item as MultiplicationTableInfo;
  ...
  protected getUsableList = (): Array<object> => {
    ...
    const kinds = this.kindTableColumnInfo.map(({ value }) => value) as Array<
      | 'numberPoker'
      | 'chinesePoker'
      | 'numberPokerFull'
      | 'chinesePokerFull'
    ...
    const usableList: Array<{
      strongI18n: I18nable;
      buttonList: Array<{ nameI18n: I18nable; info: MultiplicationTableInfo }>;
    }> = [];
    this.scopeTableColumnInfo.forEach(({ value, captions }) => {
      const scope = value as 'chinese' | 'west' | 'india';

      const buttonList: Array<{ nameI18n: I18nable; info: MultiplicationTableInfo }> = [];
      kinds.forEach((kind, index) => {
        buttonList.push({
          nameI18n: kindI18n[index],
          info: {
      ...
      const strongI18n: I18nable = captions;
      usableList.push({
        strongI18n,
        buttonList,
      });
  ...
  private appendSvgPath(
    createSvgPath: any,
    stroke: string,
    RADIUS_PX: number,
    WIDTH_SUBTRACT_DIAMETER_PX: number,
    HEIGHT_SUBTRACT_DIAMETER_PX: number,
    svg: SVGElement,
  ) {
    // const { formatMillimeter } = this;
    ...
    const path = createSvgPath() as SVGPathElement;
    path.setAttribute('fill', 'none');
    path.setAttribute('stroke', stroke);
    path.setAttribute(
      'd',
      `M 0, ${RADIUS_PX}`.concat(
  ...
  private fillIndiaNumberSeries(
    ROW_COUNT: number,
    NUMBER_SERIES: number[][],
    OTHER_NUMBER_SERIES: number[],
  ) {
    switch (ROW_COUNT) {
      case 29:
  ...
  private fillChineseNumberSeries(
    ROW_COUNT: number,
    NUMBER_SERIES: number[][],
    OTHER_NUMBER_SERIES: number[],
  ) {
