 ...
 * </zh_tw>
 */
export abstract class CopybookBase extends BrickBase {
  ...
  protected usableCopybookCheckboxElementArray: Array<
    HTMLInputElement & { words: Array<{ chinese: string; pinyin: string }> }
  > = [];
  ...
  protected usableCopybooksPeopleEducationEdition: Array<{
    termI18n: I18nable;
    units: Array<{ names: I18nable; words: Array<{ chinese: string; pinyin: string }> }>;
  }> = [];
  ...
  private isPinyinBill = (pinyinBill: string): boolean => {
  ...
  protected countDataAndComputedData = (): void => {
    const { data, computedData } = this as any;
    // const { CHARS } = this;
    const {
      paperSize,
      isLandscape,
    ...
    /*
        ...
        @page :top{margin-top:${pageMarginTop}mm;}
        @page :left{margin-left:${pageMarginLeft}mm;}
      }
    ...
    */
    ...
    /*
      ...
      =>
      page { width:${MAX_X}mm;padding-left:${pageMarginLeft}mm;padding-top:${pageMarginTop}mm; }
    */
    ...
    const CHAR_COUNT_PER_ROW = Math.floor(
      (PAGE_WIDTH * 0.5 + HORIZONTAL_SPACE) / (RECTANGLE_WIDTH + HORIZONTAL_SPACE),
    );
    ...
    const i18nSubtitle: any =
      inputMethod === 'select' && selectedCheckboxIndexArray.length === 1
        ? (
            this.usableCopybookCheckboxElementArray.filter(
      ...
      randomizedCopybooks.forEach(({ chinese, pinyin }) => {
        ...
        charArray.forEach((char, index) => {
          ...
          const pinyinHtml = `<pinyin ${
            pinyin.length > 4 ? ' chars="'.concat(pinyin.length.toString(), '"') : ''
          }>${pinyin
            .split('')
            .map((char, index) =>
              '<pinyin-char'.concat(
  ...
  protected getRandomizedCopybooks = (): Array<{ chinese: string; pinyin: string }> => {
    const {
      data: { copybooks },
    } = this as any;
    if (copybooks.length === 0) return [];

    const remaining: Array<{ chinese: string; pinyin: string }> = [];
    copybooks.forEach((copybook: any) => remaining.push(copybook));
    // console.log('getRandomizedCopybooks(), copybooks:', copybooks);
    // console.log('getRandomizedCopybooks(), remaining:', remaining);

    const result: Array<{ chinese: string; pinyin: string }> = [];
  ...
  protected updateOtherData = (newData: any): void => {
    ...
    copybooks.forEach((copybook: object) => data.copybooks.push(copybook));
  ...
  private idOrClassPrefix = 'brickPageCopybook';
  protected textareaChinese = createElement('textarea') as HTMLTextAreaElement;
  protected textareaPinyin = createElement('textarea') as HTMLTextAreaElement;
  protected textareaChineseAndPinyin = createElement('textarea') as HTMLTextAreaElement;
  public initCoreElements = (): void => {
    ...
    const booksWrap = createElement('div') as HTMLDivElement;
    booksWrap.id = `${idOrClassPrefix}UsableCopybooksWrap`;
    configCoreElement.appendChild(booksWrap);

    const detailsPeopleEducationEdition = createElement('details') as HTMLDetailsElement;
    ...
    const summaryPeopleEducationEdition = createElement('summary') as HTMLDivElement;
    ...
    const usableCopybooksPeopleEducationEditionWrap = createElement('div') as HTMLDivElement;
    // configCoreElement.appendChild(usableCopybooksWrap);
    ...
    usableCopybooksPeopleEducationEdition.forEach(
      ({
        ...
        const usableCopybookWrap = createElement('div') as HTMLDivElement;
        ...
        const spanGroup = createElement('span') as HTMLSpanElement;
        usableCopybookWrap.appendChild(spanGroup);
        spanGroup.className = `${idOrClassPrefix}UsableCopybookCheckboxGroupWrap`;

        units.forEach(
          (unit: { names: I18nable; words: Array<{ chinese: string; pinyin: string }> }) => {
            const { names, words } = unit;

            const spanWrap = createElement('span') as HTMLSpanElement;
            ...
            (checkbox as any).words = words;
            (checkbox as any).names = names;
            usableCopybookCheckboxElementArray.push(checkbox as any);
            ...
            // copybooks
            const checkboxChanged = (event: Event): void => {
              const copybooks: Array<{ chinese: string; pinyin: string }> = [];
              const chineseArray: Array<string> = [];
              const pinyinArray: Array<string> = [];
              const chineseAndPinyinArray: Array<string> = [];

              const selectedCheckboxIndexArray: Array<number> = [];

              usableCopybookCheckboxElementArray.forEach(one => {
                if (one.checked) {
                  selectedCheckboxIndexArray.push(
                    parseInt(one.getAttribute('edu-index') as string, 0),
                  );
                  one.words.forEach((chineseAndPinyinPair: { chinese: string; pinyin: string }) => {
            ...
            span.onclick = (event: Event): boolean => {
    ...
    const copybookInputWrap = createElement('div') as HTMLDivElement;
    ...
    const textareaGroupWrap = createElement('span') as HTMLSpanElement;
  ...
  private updateCopybookData = () => {
    ...
    kindElementArray.forEach((radio: HTMLInputElement) => {
      radio.checked = radio.value === kind;
    });
    inputMethodElementArray.forEach((radio: HTMLInputElement) => {
      radio.checked = radio.value === inputMethod;
    });
    fontSizeElementArray.forEach((radio: HTMLInputElement) => {
      radio.checked = radio.value === fontSize;
    });
    colorElementArray.forEach((radio: HTMLInputElement) => {
      radio.checked = radio.value === color;
    });

    usableCopybookCheckboxElementArray.forEach((checkbox: HTMLInputElement) => {
    ...
    const pinyinArray: Array<string> = [];
    const chineseAndPinyinArray: Array<string> = [];
    copybooks.forEach(({ chinese, pinyin }: { chinese: string; pinyin: string }) => {
      chineseArray.push(chinese);
      pinyinArray.push(pinyin);
  ...
  private updateChineseAndPinyinTextarea = (
    textareaChineseAndPinyin: HTMLTextAreaElement,
    textareaChinese: HTMLTextAreaElement,
    textareaPinyin: HTMLTextAreaElement,
  ): void => {
    const copybooks: Array<{ chinese: string; pinyin: string }> = [];
    ...
    for (let i = 0; i < chineseAndPinyinLength; ++i) {
      ...
      const pinyin = (pairArray.length < 2 ? '' : pairArray[1])
        .replace(/a/g, 'ɑ')
        .replace(/g/g, 'ɡ');
  ...
  private updateCopybooks = (copybooks: { chinese: string; pinyin: string }[]): void => {
    const { data } = this as any;
    data.copybooks.length = 0;
  ...
  protected onRadioOptionChanged = (propertyName: string, value: string | boolean | number) => {
    switch (propertyName) {
      case 'kind':
        break;
      case 'inputMethod':
  ...
  private appendCopybookOfGrade1Term1 = (): void => {
  ...
  private appendCopybookOfGrade1Term2 = (): void => {
    const { usableCopybooksPeopleEducationEdition } = this;
  ...
  private appendCopybookOfGrade2Term1 = (): void => {
    const { usableCopybooksPeopleEducationEdition } = this;
